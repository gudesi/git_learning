...
class Config:
    CASH_ETF = "511880.SS"
    BROAD_ETFS     = ["510300.SS", "510500.SS", "512100.SS", "563300.SS", "159915.SZ", "510880.SS"]
    ...
    BULL_REGIME_SYMBOLS = ("510300.SS", "510500.SS", "159915.SZ")
    BULL_REGIME_MA = 200
    BULL_REGIME_THRESHOLD = 2
...
def normalize_symbol(symbol: str) -> str:
    """统一 XSHG/XSHE → SS/SZ"""
    if symbol.endswith(".XSHG"): return symbol.replace(".XSHG", ".SS")
    if symbol.endswith(".XSHE"): return symbol.replace(".XSHE", ".SZ")
    return symbol
...
def _get_history_field(symbol: str, field: str, count: int) -> List[float]:
    try:
        ...
        if hasattr(data, "values") and hasattr(data, "columns"):
            if field in data.columns: return list(data[field].values)
            if len(data.columns) == 1: return list(data.iloc[:, 0].values)
...
@cached("sma")
def calc_sma(symbol: str, lookback: int) -> Optional[float]:
    close = get_close(symbol, lookback)
    return sum(close) / lookback if len(close) >= lookback else None
...
def calc_trend_quality_raw(symbol: str, lookback: int = Config.QUALITY_LOOKBACK) -> Optional[float]:
    ...
    if ss_total <= 0: return None
    ss_res = sum((log_prices[i] - (y_mean + slope * (x[i] - x_mean))) ** 2 for i in range(n))
    r_sq = 1.0 - ss_res / ss_total
    return slope * r_sq

@cached("trend_persistence")
def calc_trend_persistence_raw(symbol: str, lookback: int = Config.PERSISTENCE_LOOKBACK,
                               ma_window: int = Config.PERSISTENCE_MA_WINDOW) -> Optional[float]:
    close = get_close(symbol, lookback + ma_window)
    if len(close) < lookback + ma_window: return None
    days_above_ma = 0
    for i in range(ma_window, len(close)):
        ma = sum(close[i - ma_window:i]) / ma_window
        if close[i] > ma: days_above_ma += 1
    return days_above_ma / float(lookback)

@cached("max_drawdown")
def calc_max_drawdown_raw(symbol: str, lookback: int = Config.DRAWDOWN_LOOKBACK) -> Optional[float]:
    close = get_close(symbol, lookback)
...
def calc_trend_stability_raw(symbol: str, lookback: int = Config.STABILITY_LOOKBACK) -> Optional[float]:
    close = get_close(symbol, lookback + 1)
    if len(close) < lookback + 1: return None
    returns = []
    for i in range(1, len(close)):
        if close[i - 1] <= 0: return None
        returns.append(close[i] / close[i - 1] - 1)
    if len(returns) < 2: return None
    mean_ret = sum(returns) / len(returns)
    variance = sum((r - mean_ret) ** 2 for r in returns) / (len(returns) - 1)
    std = math.sqrt(variance)
    return 1.0 / std if std and std > 0 else None

@cached("adv")
def calc_adv60(symbol: str, lookback: int = Config.LIQUIDITY_LOOKBACK) -> Optional[float]:
    turnover = get_turnover(symbol, lookback)
    return sum(turnover) / len(turnover) if turnover and len(turnover) >= lookback else None
...
def calc_atr(symbol: str, lookback: int = Config.ATR_LOOKBACK) -> Optional[float]:
    ...
    close = get_close(symbol, lookback + 1)
    if len(high) < lookback + 1 or len(low) < lookback + 1 or len(close) < lookback + 1: return None
...
def calc_atr_percent(symbol: str) -> Optional[float]:
    ...
    return atr / close[-1] if atr is not None and close and close[-1] > 0 else None
...
def precompute_universe() -> Dict[str, Any]:
    ...
    if cache_key in GLOBAL_CACHE: return GLOBAL_CACHE[cache_key]

    risk_adj_mom, trend_q_raw, trend_persistence_raw, drawdown_raw, stability_raw, adv60_dict = {}, {}, {}, {}, {}, {}
    for sym in Config.RISK_ETFS:
        mom_dict = {}
        for lb in Config.RETURN_WINDOWS:
            val = calc_risk_adjusted_momentum(sym, lb)
            if val is not None: mom_dict[lb] = val
        risk_adj_mom[sym] = mom_dict
        if (q := calc_trend_quality_raw(sym)) is not None: trend_q_raw[sym] = q
        if (p := calc_trend_persistence_raw(sym)) is not None: trend_persistence_raw[sym] = p
        if (dd := calc_max_drawdown_raw(sym)) is not None: drawdown_raw[sym] = dd
        if (st := calc_trend_stability_raw(sym)) is not None: stability_raw[sym] = st
        if (adv := calc_adv60(sym)) is not None: adv60_dict[sym] = adv

    momentum_cross = {lb: sorted([v[lb] for v in risk_adj_mom.values() if lb in v]) for lb in Config.RETURN_WINDOWS}
    quality_cross = sorted([v for v in trend_q_raw.values() if v is not None])
    persistence_cross = sorted([v for v in trend_persistence_raw.values() if v is not None])
    drawdown_cross = sorted([v for v in drawdown_raw.values() if v is not None])
    stability_cross = sorted([v for v in stability_raw.values() if v is not None])
...
def _percentile_rank(value: float, sorted_list: List[float]) -> float:
    if len(sorted_list) <= 1: return 0.5
...
def calc_momentum_score(symbol: str, universe: Dict[str, Any]) -> Optional[float]:
    ...
    for lb in Config.RETURN_WINDOWS:
        ...
        if raw is None or cross is None: return None
...
def calc_quality_score(symbol: str, universe: Dict[str, Any]) -> Optional[float]:
    ...
    return _percentile_rank(raw, cross) if raw is not None and cross else None

def calc_persistence_score(symbol: str, universe: Dict[str, Any]) -> Optional[float]:
    raw = universe["trend_persistence_raw"].get(symbol)
    cross = universe.get("persistence_cross_section")
    return _percentile_rank(raw, cross) if raw is not None and cross else None

def calc_drawdown_score(symbol: str, universe: Dict[str, Any]) -> Optional[float]:
    raw = universe["drawdown_raw"].get(symbol)
    cross = universe.get("drawdown_cross_section")
    return _percentile_rank(raw, cross) if raw is not None and cross else None

def calc_stability_score(symbol: str, universe: Dict[str, Any]) -> Optional[float]:
    raw = universe["stability_raw"].get(symbol)
    cross = universe.get("stability_cross_section")
    return _percentile_rank(raw, cross) if raw is not None and cross else None
...
def calc_final_score(symbol, universe):
...
def get_etf_category(symbol):
    ...
    if symbol in Config.BROAD_ETFS:
        return "BROAD"

    if symbol in Config.FINANCIAL_ETFS:
        return "FIN"

    if symbol in Config.TECH_ETFS:
        return "TECH"

    if symbol in Config.DEFENSE_ETFS:
        return "DEF"

    if symbol in Config.HEALTHCARE_ETFS:
        return "HEALTH"

    if symbol in Config.CONSUMER_ETFS:
        return "CONS"

    if symbol in Config.RESOURCE_ETFS:
        return "RESOURCE"
...
def calc_inverse_volatility_weights(symbols: List[str]) -> Dict[str, float]:
    ...
    for s in symbols:
        ...
        if atr_pct and atr_pct > 0: risk_vals[s] = 1.0 / atr_pct
    if not risk_vals: return {}
...
def apply_max_position_constraint(w: Dict[str, float]) -> Dict[str, float]:
    ...
    for _ in range(20):
        ...
        for s in adj:
            if adj[s] > Config.MAX_SINGLE_POSITION_WEIGHT: adj[s] = Config.MAX_SINGLE_POSITION_WEIGHT
        if excess <= Config.EPSILON: break
...
def calc_risk_budget_usage(weights: Dict[str, float]) -> Optional[float]:
    ...
    return pvol / Config.TARGET_PORTFOLIO_RISK if pvol is not None and Config.TARGET_PORTFOLIO_RISK > 0 else None

def get_risk_scaling_factor(weights: Dict[str, float]) -> float:
    usage = calc_risk_budget_usage(weights)
    if usage is None: return 1.0
    if usage < Config.LOW_RISK_THRESHOLD: return Config.LOW_RISK_EXPOSURE
    if usage < Config.HIGH_RISK_THRESHOLD: return Config.HIGH_RISK_EXPOSURE
    if usage < Config.EXTREME_RISK_THRESHOLD: return Config.EXTREME_RISK_EXPOSURE
...
def is_bull_market() -> bool:
    bull_count = 0
    details = []
    for symbol in Config.BULL_REGIME_SYMBOLS:
        close = get_close(symbol, 1)
        if not close: continue
        current_price = close[-1]
        ma = calc_sma(symbol, Config.BULL_REGIME_MA)
        if ma is None: continue
        above_ma = current_price > ma
        details.append(f"{symbol}:{above_ma}")
        if above_ma: bull_count += 1
    bull = bull_count >= Config.BULL_REGIME_THRESHOLD
    log.info(f"P4-A BULL_REGIME bull={bull} count={bull_count} details={details}")
    return bull
...
def get_position_value(symbol: str) -> float:
    ...
    try: return float(p.amount) * float(p.last_sale_price)
    ...
    try:
        ...
        if delta > 0:
            threshold = (max(total_eq * Config.CASH_MIN_TRADE_PCT, Config.CASH_MIN_TRADE_ABS)
                         if symbol == Config.CASH_ETF else Config.STOCK_MIN_TRADE_ABS)
            if delta < threshold:
...
def rebalance(context, risk_weights: Dict[str, float], cash_weight: float):
    ...
    if cash_weight > 0: all_targets[Config.CASH_ETF] = cash_weight
    ...
    def current_weight(sym: str) -> float:
        ...
        if p is None or total_eq <= 0: return 0.0
        try: return float(p.amount) * float(p.last_sale_price) / total_eq
        except Exception: return 0.0

    sell_orders, buy_orders = [], []
    for sym in sorted(all_targets.keys()):
        diff = all_targets[sym] - current_weight(sym)
        if abs(diff) < Config.REBALANCE_TOLERANCE: continue
        if diff < 0: sell_orders.append((sym, all_targets[sym]))
        else: buy_orders.append((sym, all_targets[sym]))
...
def before_trading_start(context, data): log.info("BEFORE_TRADING")
def after_trading_end(context, data): log.info("AFTER_TRADING")
